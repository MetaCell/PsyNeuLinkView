...
export class CustomLinkWidget extends DefaultLinkWidget {
  ...
  generateLinePath(firstPoint, lastPoint) {
    ...
    const angle =
      ...
      Math.PI;
    let newX = Math.round(
      Math.cos((angle * Math.PI) / 180) * distance + firstPoint.x
    );
    let newY = Math.round(
      Math.sin((angle * Math.PI) / 180) * distance + firstPoint.y
    );
  ...
  registerParentsListener() {
    ...
    const targetNode = this.props.link.getTargetPort() ? this.props.link.getTargetPort().getParent() : null;
    let targetParentNode;
    if (targetNode) {
      targetParentNode = this.getNodeParent(targetNode);
    }

    if (sourceParentNode && targetParentNode) {
      this.registerListenerAux(
        this.getListenerID(sourceNode),
        sourceParentNode
      );
      this.registerListenerAux(
   ...
   */
  portsHaveSameParent() {
    const sourceNodePath = this.props.link
      .getSourcePort()
      .getParent()
      .getGraphPath();
    sourceNodePath.pop();

    const targetNodePath = this.props.link
    .getTargetPort() ? this.props.link
      .getTargetPort()
      .getParent()
      .getGraphPath() : [];
    targetNodePath.pop();
  ...
  render() {
    ...
    if (!sourcePort.getParent().isExpanded) {
      ...
      points[0] = getEdgePoint(
        sourcePort ? sourcePort.getCenter() : points[0].getPosition(),
        targetPort ? targetPort.getCenter() : points[1].getPosition(),
        radius,
        link
      );
    }
    ...
    let isTargetPortVisible;

    if (targetPort) {
      isTargetPortVisible =  updateLinkPoints(
    ...
    return (
      <g id={link.getID()} data-default-link-test={link.getOptions().testName}>
        {paths}
      </g>
    );
  }
...
class CustomLinkAdapter extends React.Component {
  render() {
     ...
     */
    return <CustomLinkWidget link={model} diagramEngine={engine} />;
  }
}
